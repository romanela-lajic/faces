...
class CallBackVerification(object):
    def __init__(self, frequent, val_targets, rec_prefix, image_size=(112, 112)):
        ...
        # Handle cases where val_targets is not a list
        if isinstance(val_targets, (list, tuple)):
            self.highest_acc_list: List[float] = [0.0] * len(val_targets)
        else:
            # If val_targets is a single value (e.g., int or str), treat it as a single target
            self.highest_acc_list: List[float] = [0.0]
            val_targets = [val_targets]  # Convert to a list for consistency
    ...
    def init_dataset(self, val_targets, data_dir, image_size):
        for name in val_targets:
            # Convert name to a string if it's not already
            name_str = str(name)
            print(data_dir)
            path = os.path.join(data_dir, name_str + ".bin")
            if os.path.exists(path):
                data_set = verification.load_bin(path, image_size)
                self.ver_list.append(data_set)
                self.ver_name_list.append(name_str)
...
class CallBackModelCheckpoint(object):
    ...
    def __call__(self, global_step, backbone: torch.nn.Module, header: torch.nn.Module = None, quantiza: bool = False):
        if global_step > 100:
            if quantiza:
                torch.save(backbone, os.path.join(self.output, str(global_step) + "backbone.pt"))
            else:
